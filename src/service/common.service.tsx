...
export class CommonService {
    static getAssetpath() {
        return from(
            axios.get(Constants.API_URL + constants.apiUrl.getAssetPath),
        ).pipe(
    ...
    static lockChildAccount(userID: string) {
        return RestClient.get(
            constants.apiUrl.lockChild.replace('{userID}', userID),
        );
    }

    static deleteChildAccount(userID: string) {
        return RestClient.get(
            constants.apiUrl.deleteChild.replace('{userID}', userID),
        );
    }
}
