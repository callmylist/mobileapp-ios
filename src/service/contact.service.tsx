...
export class ContactService {
    ...
    static getContactList(options: any) {
        console.log(constants.apiUrl.getContactList.replace('{isDoNotContactList}', 'false').replace('{number}', options.currentPage).replace('{limit}', options.pageSize))
        return RestClient.get(
            constants.apiUrl.getContactList.replace('{isDoNotContactList}', 'false').replace('{number}', options.currentPage).replace('{limit}', options.pageSize)
        )
    }
    ...
    static uploadContactList(fileInfo: any, hasHeader: boolean, numberColumn: string, callback: any) {
        ...
        RNFS.uploadFiles({
            ...
            if (response.statusCode == 201) {
                ...
                callback({ message: parsedBody.message + "." + parsedBody.submessage, success: false })
            }
        })
    }
}
